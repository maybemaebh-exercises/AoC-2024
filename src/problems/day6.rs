...
fn part2_multithreaded(input: &str) -> usize {
    ...
    for _ in 0..thread::available_parallelism().unwrap().get()/2 {
        threads.push(
...
impl CharGrid<AsciiString> {
    ...
    fn loops_at(&self, location: Ucoord, direction: Direction, barrier: Option<Ucoord>, previus_turns: &mut HashSet<u24>) -> bool {//2^24/2^2 = 2^22Bytes max input before overflow (4MiB)
        previus_turns.clear();
        let mut current_location = location;
        let mut current_direction = direction;
        let offset_for_direction:usize = self.bounds[0]*self.bounds[1];
        assert!(offset_for_direction<(2usize.pow(22)));
        loop {
    ...
    fn hashmap_key_from_state(&self, current_location: Ucoord, current_direction: &Direction, offset_for_direction: usize) -> u24 {
        ((self.index_usize(current_location).unwrap()
            +
