...
impl TrackingAllocatorData {
    ...
    fn end_tracking(mut guard: SpinlockGuard<Self>) -> (isize,usize) {
        let peak = guard.peak;
        let count = guard.allocation_count;
        guard.limit = 0;
        // Cannot drop it yet, as it would trigger a deallocation
        let old_handler = guard.failure_handler.take();
        drop(guard);
    ...
    fn track_and_check_limits(
        ...
        if is_allocation {guard.allocation_count += 1}
        guard.current += alloc;
...
static ALLOCATOR_DATA: Spinlock<TrackingAllocatorData> =
    Spinlock::new(TrackingAllocatorData { current: 0, peak: 0, limit: 0, failure_handler: None, allocation_count: 0 });
...
impl<A: GlobalAlloc> TrackingAllocator<A> {
    ...
    pub fn end_tracking(&self) -> (isize, usize) {
...
unsafe impl<A: GlobalAlloc> GlobalAlloc for TrackingAllocator<A> {
    ...
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        if let Some(guard) =
            TrackingAllocatorData::track_and_check_limits(guard, layout.size() as isize,true)
        {
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        let guard = ALLOCATOR_DATA.lock();
        TrackingAllocatorData::track_and_check_limits(guard, -(layout.size() as isize), false);
